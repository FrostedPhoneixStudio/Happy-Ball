...
signal collided(platform, collider)
...
@export var can_spawn_collecables = true
var wrap_around = true

var point_counted = false

func _ready():
	# set the collision layer to false from the start so the player wont collide 
...
func _physics_process(delta):
	...
	if ball:
		if ball.global_position.y < global_position.y and !get_collision_layer_value(1):
			if get_parent() is CombinedPlatform:
				pass
